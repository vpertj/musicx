...
void main() {
  ...
  test('e2e: bilibili official plugin installs and searches (best-effort)', () async {
    final tmp = Directory.systemTemp.createTempSync('musicx_bili');
    addTearDown(() => tmp.deleteSync(recursive: true));

    final manager = PluginManager(tmp);
    const url = 'https://gitee.com/maotoumao/MusicFreePlugins/raw/v0.1/dist/bilibili/index.js';
    ...
    for (var attempt = 1; attempt <= 3; attempt++) {
      try {
        final result = await manager.search('周杰伦', timeout: const Duration(seconds: 25));
        ...
        if (data.isNotEmpty) {
          ...
          print('BILI search OK: ${data.length} results, first=${first['title']}');
          // 播放地址尽力解析
          try {
